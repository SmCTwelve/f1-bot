...
async def status(ctx, *args):
    ...
    embed.add_field(name='Owner', value=app_info.owner.name, inline=False)
    embed.add_field(name='Ping', value=latency, inline=True)
    embed.add_field(name='Uptime', value=f'{uptime[0]}d, {uptime[1]}h, {uptime[2]}s', inline=True)
    embed.add_field(
        name='Websocket',
        value=ws_conn,
        inline=True
